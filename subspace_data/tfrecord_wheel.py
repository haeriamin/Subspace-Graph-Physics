...
def serialize(data):
    ...
    #         bytes_list=tf.train.BytesList(value=[data['feature0'][i]]))
    #     id_feat.append(id_feature)
    # idp = tf.train.FeatureList(feature=id_feat)
    ...
    # Global feature
    step_con = []
    for i in range(len(data['feature3'])):
        step_context = tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[data['feature3'][i]]))
        step_con.append(step_context)
    step = tf.train.FeatureList(feature=step_con)
...
def write_json(data_path, bounds, frames, dimension, frame_rate,
    ...
    data['context_mean'] = context_mean
    data['context_std'] = context_std
    # added:
