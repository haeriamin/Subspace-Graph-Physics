...
`python -m subspace_data.run_tfrecord_excav`
...
if __name__ == '__main__':
                    ...
                    # if speed != 0.01:  # exclude 50 bad cases
                        ...
                        for frame in range(1, frames+1):
                            ...
                            for particle in range(1, all_particles):
                                if ~np.isnan(float(data_frame['col1'][particle])):
                                    if (int(data_frame['col0'][particle]) == 0) and (particle%5 == 0) and (particle not in particle_id_frame):
    ...
    #                     else:
    #                         particle_vel_mean = np.divide(np.add(particle_vel_mean_old, particle_vel_mean_case), 2)
    #                         particle_acc_mean = np.divide(np.add(particle_acc_mean_old, particle_acc_mean_case), 2)
    #                         rigid_body_force_mean = np.divide(np.add(rigid_body_force_mean_old, rigid_body_force_mean_case), 2)
    #                         if (n1_v + n2_v) > 0:
